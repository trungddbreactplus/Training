...
def node_cls(state: AgentState) -> AgentState:
    chain = cls_prompt | llm| CommaSeparatedListOutputParser()
    response = chain.invoke({
        "description": FULL_DESCRIPTION,
        "examples": FEW_SHOT_EXAMPLES,
        "user_query": state['current_query']
    })
    state['category_query'] = response
    return state

input_state = AgentState(current_query='Nhà tôi nghèo, có nhận được học bổng không')
res = node_cls(input_state)
print(res['category_query'])
